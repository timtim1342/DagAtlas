...
## Introduction and typological background
...
There are 18 such languages (see [Table 1](#m1)). Note that Northern Akhvakh and Botlikh feature markers that are described in grammars as translative markers. However, their semantics is most often elative, so they are also included in this group.

#### Table 1. Elative markers {#m1}

|Family > Branch|Language        |Suffix                   |
|---------------|----------------|-------------------------|
|EC > Avar-Andic|Avar (Std.)     |_-da-**sa**_             |
...
Five languages feature **ablative** markers (see [Table 2](#m2)). We use the term “ablative” instead of “elative” for languages that do not have bimorphemic spatial case systems, cf. Standard Azerbaijani (@three). On maps, however, elative and ablative markers are merged into a single group.
...
|                |Nogai             |_-den_       |
|Turkic > Oghuz  |Azerbaijani (Std.)|_-dan (-dən)_|
|IE > Armenic    |Armenian          |_-ic'_       |

(@three) Standard Azerbaijani (@shiralievsevortyan1971: 47)
<!--html_preserve-->
<ul>
<table> 
  <tr>
...
Five languages mark the standard of comparison with **essive** suffixes, cf. example (@five) from Tindi. Essives are usually zero-marked, i.e. no overt directionality marker follows the localization marker. In Dargwa languages, the localization marker is followed by noun class (gender) markers when the noun form is essive (see [Table 3](#m3).
...
The most common localization marker in spatial forms encoding the standard of comparison is **SUPER** (location on a surface),[^6] cf. (@two). It is found in 17 languages (see [Table 5](#m5)).

#### Table 5. SUPER markers {#m5}

|Family > Branch|Language        |Suffix              |
|---------------|----------------|--------------------|
|EC > Avar-Andic|Avar (Std.)     |_-**da**-sa_        |
...
The localization marker **AD** (location in proximity to some entity) is used in Kryz (Lezgic < EC), cf. (@seven), whereas in Bats (Nakh < EC) we find **IN** (location inside some entity), cf. (@eight).
